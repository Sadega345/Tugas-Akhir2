...
class ManageFakultas extends AUTH_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('M_ManageFakultas');
	}

	public function index() {
		$data['userdata'] = $this->userdata;

		$data['page'] = "manageFakulatas";
		$data['judul'] = "Manage Data Fakultas";
		$data['deskripsi'] = "Manage Data Fakultas";
		$data['dataFakultas'] = $this->M_ManageFakultas->select_fakultas();
		
		$data['modal_tambah_manageFakultas'] = show_my_modal('modals/modal_tambah_manageFakultas', 'tambah-manageFakultas', $data);

		$this->template->views('ManageFakultas/home', $data);
	}

	public function tampil() {
		$data['datamanageFakultas'] = $this->M_ManageFakultas->select_fakultas();
		
		$this->load->view('ManageFakultas/list_data', $data);
	}

	public function prosesTambah() {
		$this->form_validation->set_rules('nmfakultas', 'Nama Fakultas', 'trim|required');

		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_ManageFakultas->insert($data);
	...
	public function update() {
		...
		$data['dataManageFakultas'] = $this->M_ManageFakultas->select_by_id($id);
		
		$data['userdata'] = $this->userdata;

		echo show_my_modal('modals/modal_update_managefakultas','ubah-managefakultas' ,$data);
	...
	public function prosesUpdate() {
		...
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_ManageFakultas->update($data);
			...
			if ($result > 0) {
				...
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Fakultas Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_ManageFakultas->delete($id);
