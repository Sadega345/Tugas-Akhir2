...
class MasterButir extends AUTH_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('M_MasterButir');
	}

	public function index() {
		$data['userdata'] = $this->userdata;

		$data['page'] = "masterButir";
		$data['judul'] = "Master Data Butir";
		$data['deskripsi'] = "Master Data Butir";
		$data['dataButir'] = $this->M_MasterButir->select_butir();
		$data['dataStandar'] = $this->M_MasterButir->select_standar();
		$data['dataTypeBorang'] = $this->M_MasterButir->select_typeborang();
		$data['dataStudy'] = $this->M_MasterButir->select_study();
		$data['modal_tambah_masterButir'] = show_my_modal('modals/modal_tambah_masterbutir', 'tambah-masterButir', $data);

		$this->template->views('MasterButir/home', $data);
	}

	public function tampil() {
		$data['datamasterButir'] = $this->M_MasterButir->select_butir();
		
		$this->load->view('MasterButir/list_data', $data);
	}

	public function prosesTambah() {
		$this->form_validation->set_rules('namabutir', 'Nama Butir', 'trim|required');
		$this->form_validation->set_rules('nmstandar', 'Nama Standar', 'trim|required');
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_MasterButir->insert($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Butir Berhasil ditambahkan', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Data Butir Gagal ditambahkan', '20px');
			}
		} else {
			$out['status'] = 'form';
	...
	public function update() {
		...
		$data['dataMasterButir'] = $this->M_MasterButir->select_by_id($id);
		...
		echo show_my_modal('modals/modal_update_masterbutir','ubah-masterbutir' ,$data);
	...
	public function prosesUpdate() {
		...
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_MasterButir->update($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Butir Berhasil diupdate', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Butir Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_MasterButir->delete($id);

		if ($result > 0) {
			echo show_succ_msg('Data Butir Berhasil dihapus', '20px');
		} else {
			echo show_err_msg('Data Butir Gagal dihapus', '20px');
		}
	}
